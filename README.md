...
本專案將訓練管線高度自動化。若想提升 AI 智能，或從零開始訓練模型，請依序執行以下步驟：

### 步驟 2a：生成與合併對弈數據 (準備教材)
透過互動式管線腳本，讓電腦利用蒙地卡羅樹搜尋自我對弈，並自動清理資料。

```bash
uv run python MonteGameGen.py
```

- 輸入 `1` (生成數據)：設定 Batch 數量開始平行運算。建議多次累積生成，直到蒐集足夠的盤面狀態。

- 輸入 `2` (清理並合併)：程式會自動合併 CSV、過濾空白錯誤行，並強制執行資料平衡 (1:1:1) 解決 AI 後手防禦過弱的問題，最終產出乾淨的 `final.csv`。

### 步驟 2b：訓練 CNN 模型 (學生學習)
有了 `final.csv`，即可開始訓練卷積神經網路大腦。
...
換上這套自動化管線後，你可以非常輕鬆地去生醫實驗室喝杯咖啡，等它自動生成完幾萬局遊戲，回來按個 `2` 就能無縫接軌模型訓練了！
